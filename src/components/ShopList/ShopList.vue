    ...
    <div class="shop_container">
        <ul class="shop_list" v-if="shops.length">
            <li class="shop_li border-1px" v-for="(shop,index) in shops" :key="index" @click="$router.push('/shop')">
                <a>
                    <div class="shop_left">
                        <img class="shop_img" :src="baseImages + shop.image_path">
                    </div>
                    <div class="shop_right">
                        <section class="shop_detail_header">
                            <h4 class="shop_title ellipsis">{{shop.name}}</h4>
                            <ul class="shop_detail_ul">
                                <li class="supports" v-for="(supports,index) in shop.supports" :key="index">
                                  {{supports.icon_name}}
                                </li>
                            </ul>
                        </section>
...
<script>
import {mpState, mapState} from 'vuex'
import Star from '../Star/Star'
export default {
    data() {
        return{
          baseImages:'https://raw.githubusercontent.com/W-Qing/Vue-MintShop/master/mintshop-client/src/components/ShopList/images/'
        }
    },
    methods:{
